module.exports = {
    ...
    suppotivePlan: async function(){
        const pickers = await browser.$$('.tcard');
        let desiredPicker;
        for (let picker of pickers) {
            const titleElement = await picker.$('.tcard-title');
            const titleText = await titleElement.getText();
            if (titleText.includes('Supportive')) {
                desiredPicker = picker;
                break;
            }
        }
        if (desiredPicker) {
            await desiredPicker.click();
            await browser.waitUntil(async () => {
                const className = await desiredPicker.getAttribute('class');
                return className.includes('active');
            }, {
                timeout: 5000,
                timeoutMsg: 'Expected picker to be active after click'
            });
            return desiredPicker;
        } else {
            throw new Error('Supportive picker not found');
        }
    },
